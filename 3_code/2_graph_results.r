...
get_graph <- function(df, y_column, g_title, x_text, y_text, name){
  ...
  graph <- ggplot(df, aes(x = Initial_mixture, y = y_column, fill = Maintain_mixture)) +
    ...
    theme(
      ...
      axis.title.x = element_text(size = 12),  # Tamaño del texto del eje x
      axis.title.y = element_text(size = 12),  # Tamaño del texto del eje y
      strip.text = element_text(size = 14)  # Tamaño del texto de las etiquetas de los grupos
    ) +
    labs(title = g_title,
         y = y_text,
         x = x_text,
...
combined_plot_annotated <- annotate_figure(combined_plot,
                                           bottom = text_grob("Initial mixture", size = 14))
